...
@Component("intelinkRedisService")
public class IntelinkRedisServiceImpl implements RedisService {

	@Autowired
	@Qualifier("intlink_redisTemplate")
	private StringRedisTemplate intelinkRedisTemplate;

	// @Autowired
	// public void setRedisTemplate(StringRedisTemplate intlink_redisTemplate) {
	// this.intelinkRedisTemplate = intlink_redisTemplate;
	// }

	public <T> void put(String key, T obj) {
		intelinkRedisTemplate.opsForValue().set(key, JsonUtils.toJson(obj));
	}
	...
	public <T> void put(String key, T obj, int timeout, TimeUnit unit) {
		intelinkRedisTemplate.opsForValue().set(key, JsonUtils.toJson(obj), timeout, unit);
	}

	public <T> T get(String key, Class<T> cls) {
		return JsonUtils.fromJson(intelinkRedisTemplate.opsForValue().get(key), cls);
	}

	public <E, T extends Collection<E>> T get(String key, Class<E> cls, Class<T> collectionClass) {
		return JsonUtils.fromJson(intelinkRedisTemplate.opsForValue().get(key), cls, collectionClass);
	}
	...
	public void delKeyByPrefix(String prefix) {
		...
		Set<String> keys = intelinkRedisTemplate.keys(prefix + "*");
		intelinkRedisTemplate.delete(keys);
	}
	...
	public void delKeyBySuffix(String suffix) {
		...
		Set<String> keys = intelinkRedisTemplate.keys("*" + suffix);
		intelinkRedisTemplate.delete(keys);
	}

	public void hput(String key, String hashKey, Object value) {
		HashOperations<String, String, String> stringObjectObjectHashOperations = intelinkRedisTemplate.opsForHash();
	...
	public Long hdel(String key, String hashKey) {
		return intelinkRedisTemplate.opsForHash().delete(key, new Object[] { hashKey });
	}

	public boolean exists(String key) {
		return intelinkRedisTemplate.hasKey(key);
	}

	public void delete(String key) {
		intelinkRedisTemplate.delete(key);
	}

	public boolean expire(String key, long timeout, TimeUnit timeUnit) {
		return intelinkRedisTemplate.expire(key, timeout, timeUnit);
	}
	...
	public void put(String key, String value) {
		intelinkRedisTemplate.opsForValue().set(key, value);
	}
	...
	public void put(String key, String value, int timeout, TimeUnit unit) {
		intelinkRedisTemplate.opsForValue().set(key, value, timeout, unit);
	}

	public String get(String key) {
		return intelinkRedisTemplate.opsForValue().get(key);
	}
